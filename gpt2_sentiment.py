...
def reformat_data(args, sst_data, tokenizer):

    print("Building SST dataset")

    dataset = {"input_ids": []}

    for i, line in enumerate(sst_data):
        ...
        if args.use_label_only:
            dataset["input_ids"].append(tokenizer.encode(label + text))
        else:
            dataset["input_ids"].append(tokenizer.encode(domain_token + label + text))
...
class SSTModel(nn.Module):
    ...
    def forward(
        ...
        if self.use_mlp:
            domain_emb = self.lm_model.transformer.wte(input_ids[:, 0].unsqueeze(1))  # bze x emb_size
            label_emb = self.lm_model.transformer.wte(input_ids[:, 1].unsqueeze(1))  # bze x emb_size
            ...
            if self.use_label_only:
                label_logits, label_past = self.lm_model(input_ids[:, 0].unsqueeze(1))
                ...
                domain_logits, domain_past = self.lm_model(input_ids[:, 0].unsqueeze(1))
                label_logits, label_past = self.lm_model(input_ids[:, 1].unsqueeze(1))
                # print("label")
        ...
        if self.use_label_only:
            transfered_past = label_transfered_past
        else:
        ...
        if self.use_label_only:
            input_shape = input_ids[:, 1:-1].size()
        else:
            input_shape = input_ids[:, 2:-1].size()
        position_ids = torch.arange(0, input_shape[-1], dtype=torch.long, device=device)
        position_ids = position_ids.unsqueeze(0).view(-1, input_shape[-1])

        if self.use_label_only:
            lm_logits, lm_past = self.lm_model(input_ids[:, 1:-1], past=transfered_past, position_ids=position_ids)
        else:
            lm_logits, lm_past = self.lm_model(input_ids[:, 2:-1], past=transfered_past, position_ids=position_ids)
        ...
        if self.use_label_only:
            loss = self.criterion(lm_probs, input_ids[:, 2:].unsqueeze(-1), masks[:, 2:].unsqueeze(-1))
        else:
            loss = self.criterion(lm_probs, input_ids[:, 3:].unsqueeze(-1), masks[:, 3:].unsqueeze(-1))

        if self.bayes:
            domain_lm_logits, domain_lm_past = self.lm_model(input_ids[:, 2:-1], past=domain_transfered_past,
                                                             position_ids=position_ids)
            domain_lm_probs = self.logsoftmax(domain_lm_logits)
            domain_lm_loss = self.criterion(domain_lm_probs, input_ids[:, 3:].unsqueeze(-1), masks[:, 3:].unsqueeze(-1))
            loss += domain_lm_loss
        ...
        if self.kl_scale > 0.0:
            ori_logits, _ = self.lm_model(input_ids[:, 2:-1], past=None)
...
def train(args, tokenizer, model):
    ...
    with open(args.train_data) as train_sst_data:
        if "formatted" in args.train_data:
            train_formatted_data = json.load(train_sst_data)
        else:
            # train_sst_data = json.load(json_file)
            train_formatted_data = reformat_data(args, train_sst_data, tokenizer)
            ...
            f.write(json_file)
            f.close()
        train_dataset = SSTDataset(train_formatted_data, gpt2_padding_value)
...
def main():
    ...
    model = SSTModel(lm_model, config, args.device, args.kl_scale, args.bayes, args.use_mlp)
