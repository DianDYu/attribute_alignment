...
from topic_finetune_training import TOPICModel
...
zero_shot_label_tokens = ["religion", "legal", "politics", "military", "computer", "space"]
SMALL_CONST = 1e-5
...
def generate(model, input_ids, label_ids, cur_len, max_length, do_sample, temperature, top_k, top_p,
    ...
    with torch.no_grad():
        ...
        cur_token = torch.cat((label_ids, input_ids), dim=-1)
        ...
        while cur_len < max_length:
            ...
            logits, past = model.lm_model(cur_token, past=past)

            next_token_logits = logits[:, -1, :-3]  # the last 3 tokens are special tokens, not considered during generation
...
def get_gpt1_ppl(args, generated_sentence, gpt1_model, gpt1_tokenizer):
    with torch.no_grad():
        input_ids = torch.LongTensor(gpt1_tokenizer.encode(generated_sentence)).unsqueeze(0)
...
def get_style_prob(args, generated_sentence, style_eval_model, style_eval_tokenizer):
    if args.domain_token == "wiki":   # 0 for movie, 1 for wiki, 2 for news
        style_idx = 1
    elif args.domain_token == "news":
        style_idx = 2
    ...
    with torch.no_grad():
        ...
        logits = outputs[0]
        probs = F.softmax(logits, dim=-1).squeeze(0)
        pred_prob = probs[style_idx]  # 0 for movie, 1 for wiki, 2 for news
        return pred_prob.item()
...
def generate_results(args, prompt_text, model, tokenizer, total_added_tokens, eval_model, eval_tokenizer,
    ...
    if args.domain_token == "wiki":
        label_classes = dbpedia_label_tokens
        domain_token = wiki_domain_token
    elif args.domain_token == "news":
        label_classes = news_label_tokens
        domain_token = wiki_domain_token
    else:
        assert False, "args.domain_token not expected"

    if args.zero_shot:
        label_classes += zero_shot_label_tokens

    batch_size = input_ids.shape[0]

    all_label_ids = dict()
    for c in label_classes:
        label_ids = torch.LongTensor(tokenizer.encode(c)).unsqueeze(0).to(args.device)
        label_ids_len = label_ids.size()[1]
        if args.num_return_sequences > 1:
            label_ids = label_ids.expand(batch_size, args.num_return_sequences, label_ids_len)
            label_ids = label_ids.contiguous().view(batch_size * args.num_return_sequences, label_ids_len)
        all_label_ids[c] = label_ids
    ...
    #
    # if args.do_eval:
    #     ori_on_topic_prob = dict()
    #     all_ori_on_topic_sent = dict()
    #     all_ori_style_prob = dict()
    #     original = False
    #     print("\n\n")
    #     for label_idx, label_name in enumerate(label_classes):
    #         ppl, ori_class_prob, ori_on_topic_sent, cur_ori_all_ppl, ori_style_prob = eval(args, ori_generated_sequence, model.lm_model,
    #                                                       tokenizer, eval_model, eval_tokenizer,
    #                                                       label_name, label_idx, total_added_tokens, original=original,
    ...
    #         ori_on_topic_prob[label_name] = ori_class_prob
    #         all_ori_on_topic_sent[label_name] = ori_on_topic_sent
    #         all_ori_style_prob[label_name] = ori_style_prob
    #         # print("++++"*30)
    #         # if not original:
    #         #     print("original ppl: %.4f" % ori_ppl)
    #         # print(label_name)
    #         # print(ori_class_prob)
    #         # print()
    #
    #         original = True

    if args.domain_token == "news":
        ori_on_topic_prob = {"world": 0, "business": 0, "sports": 0, "science technology": 0}
        all_ori_on_topic_sent = {"world": 0, "business": 0, "sports": 0, "science technology": 0}
        all_ori_distinct = [0, 0, 0]
        ori_ppl = 0
        all_ori_style_prob = {"world": 0, "business": 0, "sports": 0, "science technology": 0}
    else:
        ori_on_topic_prob = {'company': 0, 'educational institution': 0, 'artist': 0, 'athlete': 0, 'officeholder': 0, 'means of transportation': 0, 'building': 0, 'natural place': 0, 'village': 0, 'animal': 0, 'plant': 0, 'album': 0, 'film': 0, 'written work': 0}
        all_ori_on_topic_sent = {'company': 0, 'educational institution': 0, 'artist': 0, 'athlete': 0, 'officeholder': 0, 'means of transportation': 0, 'building': 0, 'natural place': 0, 'village': 0, 'animal': 0, 'plant': 0, 'album': 0, 'film': 0, 'written work': 0}
        all_ori_distinct = [0, 0, 0]
        ori_ppl = 0
        all_ori_style_prob = {'company': 0, 'educational institution': 0, 'artist': 0, 'athlete': 0, 'officeholder': 0, 'means of transportation': 0, 'building': 0, 'natural place': 0, 'village': 0, 'animal': 0, 'plant': 0, 'album': 0, 'film': 0, 'written work': 0}

    all_label_on_topic_prob = dict()
    all_style_prob = dict()
    all_label_ppl = dict()
    all_on_topic_sent = dict()
    all_label_all_ppl = dict()
    all_sent_distinct = dict()
    for label_name, label_ids in all_label_ids.items():
        #
        # # WARNING: For GPT2 baseline running only
        # all_label_on_topic_prob[label_name] = 0
        # all_on_topic_sent[label_name] = ""
        # all_label_ppl[label_name] = 0
        # all_label_all_ppl[label_name] = [0] * 30
        # all_sent_distinct[label_name] = [0, 0, 0]
        # continue
        ...
        if args.print_generation:
            ...
            print("====" * 30)
        print(label_name)
        generated_sequence = output_generation(prompt_text, model, input_ids, label_ids, cur_len, max_len, do_sample,
                                               args.temperature, args.k, args.p, args.repetition_penalty, batch_size,
                                               tokenizer, args.use_domain_tag, args.use_label_tag, domain_ids,
                                               args.bayes, args.bayes_gamma, args.use_mlp, args.bayes_lambda)
        distinct1, distinct2, distinct3 = eval_distinct(generated_sequence, tokenizer)
        all_sent_distinct[label_name] = [distinct1, distinct2, distinct3]
        ...
        if args.do_eval:
            label_idx = label_classes.index(label_name)
            label_ppl, label_class_prob, on_topic_sent, label_all_ppl, style_prob = eval(args, generated_sequence, model.lm_model, tokenizer,
                                                              eval_model, eval_tokenizer,
                                                              label_name, label_idx, total_added_tokens,
                                                              gpt1_model=gpt1_model, gpt1_tokenizer=gpt1_tokenizer,
                                                                                         style_eval_model=style_eval_model,
                                                                                         style_eval_tokenizer=style_eval_tokenizer)
            all_label_on_topic_prob[label_name] = label_class_prob
            all_on_topic_sent[label_name] = on_topic_sent
            all_label_ppl[label_name] = label_ppl
            all_label_all_ppl[label_name] = label_all_ppl
            all_style_prob[label_name] = style_prob
    ...
    if args.do_eval:
        if args.output_results:
            ...
            output_file.write(str(ori_on_topic_prob))
            output_file.write("\n")
            output_file.write("Trained model ppl\n")
            output_file.write(str(all_label_ppl))
            output_file.write("\n")
            output_file.write("Trained model probs\n")
            output_file.write(str(all_label_on_topic_prob))
            output_file.write("\n")
            output_file.write("<<<" * 30)
            output_file.write("\n")
            output_file.write("original on topic sents\n")
            for ori_label_class, ori_label_sent in all_ori_on_topic_sent.items():
                output_file.write("=== GENERATED SEQUENCE {} ===\n".format(ori_label_class))
                output_file.write(ori_label_sent)
                output_file.write("\n")
            output_file.write("\n")
            output_file.write(">>>" * 30)
            output_file.write("\n")
            output_file.write("on topic sents\n")
            for label_class, label_sent in all_on_topic_sent.items():
                output_file.write("=== GENERATED SEQUENCE {} ===\n".format(label_class))
                output_file.write(str(label_sent))
                output_file.write("\n")
            output_file.write("\n"*10)
        else:
            print("\n\n")
            ...
            print(ori_on_topic_prob)
            print("Trained model ppl")
            print(all_label_ppl)
            print("Trained model probs")
            print(all_label_on_topic_prob)
            print()
            print("<<<" * 30)
            print("original on topic sents")
            for ori_label_class, ori_label_sent in all_ori_on_topic_sent.items():
                print("=== GENERATED SEQUENCE {} ===".format(ori_label_class))
                print(ori_label_sent)
            print()
            print(">>>" * 30)
            print("on topic sents")
            for label_class, label_sent in all_on_topic_sent.items():
                print("=== GENERATED SEQUENCE {} ===".format(label_class))
                print(label_sent)
            print()
    ...
    ori_ppl_stdev = 0
    label_all_ppl_list = []
    for c, all_ppl in all_label_all_ppl.items():
        # print(c)
        # print(all_ppl)
        c_mean = statistics.mean(all_ppl)
        c_stdev = statistics.stdev(all_ppl)
        print("attribute %s: mean: %.4f; stdev: %.4f" % (c, c_mean, c_stdev))
        label_all_ppl_list += all_ppl
    model_ppl_mean = statistics.mean(label_all_ppl_list)
    model_ppl_stdev = statistics.stdev(label_all_ppl_list)
    print("original mean: %.4f, stdev: %.4f" % (ori_ppl_mean, ori_ppl_stdev))
    print("model mean: %.4f; stdev: %.4f" % (model_ppl_mean, model_ppl_stdev))

    return ori_on_topic_prob, ori_ppl, all_label_on_topic_prob, all_label_ppl, all_ori_distinct, all_sent_distinct, \
...
def main():
    ...
    parser.add_argument("--domain_token", type=str, default=None, help="which domain is used for eval")
    ...
    parser.add_argument("--zero_shot", action="store_true", help="zero-shot controlling label")
    ...
    model = TOPICModel(lm_model, config, args.device, 0, args.bayes, args.use_mlp, args.old_d_l)
    ...
    for prompt_text in args.prompt:
        ...
        print("###" * 30)
        print(prompt_text)
        ori_on_topic, ori_ppl, on_topic, ppl, all_ori_distinct, all_sent_distinct, ori_style, model_style = generate_results(
            args, prompt_text, model, tokenizer, total_added_tokens, eval_model, eval_tokenizer,
            gpt1_model, gpt1_tokenizer, style_eval_model, style_eval_tokenizer)
        ...
        for c in label_classes:
            ...
            overall_distinct[c].append(all_sent_distinct[c])
...
"""ython topic_ft_generation.py --model_name_or_path gpt2-medium --load_checkpoint models/sst_gpt2/topic_news_ft/checkpoint-2 --length 40 --num_return_sequences 30 --repetition_penalty 1 --do_eval --eval_model_path transformers/dev_news_classification/ --eval_ppl_gpt2_large --do_style_eval --style_eval_model_path transformers/style_classification/ --prompt "In summary", "This essay discusses", "Views on", "The connection", "Foundational to this is", "To review,", "In brief,", "An illustration of", "Furthermore,", "The central theme", "To conclude,", "The key aspect", "Prior to this", "Emphasised are", "To summarise", "The relationship", "More importantly,", "It has been shown", "The issue focused on", "In this essay" """
