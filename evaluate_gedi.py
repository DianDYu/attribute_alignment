        ...
        with open(real_file_name) as f:
            ...
            for line in f:
                if line.startswith("<bos>"):
                    ...
                    generated_text = line[5:]
                else:
                    generated_text += line
            all_prompt_lines[filename].append(generated_text.strip())
        if "adv_pos" in filename or "adv_neg" in filename:
            pass
        elif "positive" in filename or "negative" in filename:
            assert len(all_prompt_lines[filename]) == 150, "len %d  != 10" % len(all_prompt_lines[filename])
        else:
            assert len(all_prompt_lines[filename]) == 200, "len %d  != 10" % len(all_prompt_lines[filename])
...
def main():
    ...
    if "neg" in args.file_path:
        ...
        class_idx = 0
    # for topic
    elif "world" in args.file_path:
        class_idx = 0
    elif "sports" in args.file_path:
        class_idx = 1
    elif "business" in args.file_path:
        class_idx = 2
    elif "science" in args.file_path:
        class_idx = 3
    else:
        assert False
    ...
    for filename, generated_sequences in all_prompt_lines.items():
        print(filename)
        file_prompts = dict()
        on_topic_sentence = ""
        ...
        for generated_sentence in generated_sequences:
            ...
            current_prompt = generated_sentence[:7]
            if current_prompt not in file_prompts:
                file_prompts[current_prompt] = [0, ""]
            ...
            if class_prob > file_prompts[current_prompt][0]:
                file_prompts[current_prompt] = [class_prob, generated_sentence]
...
"""CUDA_VISIBLE_DEVICES=1 python evaluate_gedi.py --eval_model_path transformers/imdb_classification/ --file_path gedi_data/positive/ --eval_ppl_gpt2_large --style_eval_model_path transformers/style_classification"""
"""CUDA_VISIBLE_DEVICES=1 python evaluate_gedi.py --eval_model_path transformers/dev_news_classification/ --file_path gedi_data/world/ --eval_ppl_gpt2_large --style_eval_model_path transformers/style_classification"""
