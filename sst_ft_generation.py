...
from sst_finetune_training import SSTModel
...
def get_gpt1_ppl(args, generated_sentence, gpt1_model, gpt1_tokenizer):
    with torch.no_grad():
        input_ids = torch.LongTensor(gpt1_tokenizer.encode(generated_sentence)).unsqueeze(0).to(1)
...
def generate_results(args, prompt_text, model, tokenizer, total_added_tokens, eval_model, eval_tokenizer,
    ...
    #         ppl, ori_sent_prob, ori_sent_sent, cur_ori_all_ppl = eval(args, ori_generated_sequence, model.lm_model,
    ...
    all_ori_sent_prob = {"positive": 0, "negative": 0}
    all_ori_sent_sent = {"positive": "0", "negative": "0"}
    all_ori_distinct = [0, 0, 0]
    ori_ppl = 0
    all_ori_style_prob = {"positive": 0, "negative": 0}
    ...
    print("original mean: %.4f, stdev: %.4f" % (0, 0))
