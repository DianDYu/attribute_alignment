...
from gpt2_topic_train import TOPICModel
...
def concat_past(ori_past, new_past):
    num_layers = len(ori_past)
    concated_past = list()
...
def generate(model, tokenizer, input_ids, label_ids, cur_len, max_length, do_sample, temperature, top_k, top_p,
    ...
    with torch.no_grad():
        if (bayes and bayes_gamma > -1) or (use_domain_tag and use_label_tag):
            if use_mlp:
                ...
                # # WARNING: changed for multiple attributes for testing
                label_ids_1 = torch.LongTensor(tokenizer.encode("business")).unsqueeze(0).to("cuda").repeat(label_ids.shape[0], 1)
                label_logits_1, label_past_1 = model.lm_model(label_ids_1, past=None)
                label_transfered_past_1 = tuple(
                    model.transfer_past(label_past_l, past_transfer_l) for (label_past_l, past_transfer_l) in
                    zip(label_past_1, model.label_past_transfer))
                label_transfered_past = concat_past(label_transfered_past, label_transfered_past_1)
        ...
        while cur_len < max_length:
            ...
            if bayes and (bayes_gamma > -1 and generated_length < bayes_gamma):
                ...
                d_next_token_logits = d_logits[:, -1, :-3]
...
def main():
    ...
    model = TOPICModel(lm_model, config, args.device, 0, args.bayes, args.use_mlp)
    ...
    if args.do_eval:
        assert args.zero_shot is False, "zero_shot needs to be False for on topic evaluation"
    ...
    for prompt_text in args.prompt:
        ...
        print("###" * 30)
        print(prompt_text)
        ori_on_topic, ori_ppl, on_topic, ppl, all_ori_distinct, all_sent_distinct, ori_style, model_style = generate_results(args, prompt_text, model, tokenizer, total_added_tokens, eval_model, eval_tokenizer,
                         gpt1_model, gpt1_tokenizer, style_eval_model, style_eval_tokenizer)
